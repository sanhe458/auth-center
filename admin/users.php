...
// 操作：禁用/启用/设管理员/撤销管理员/余额调整
$action = $_POST['action'] ?? '';
$uid = (int)($_POST['uid'] ?? 0);
if ($action && $uid) {
    // 不能操作自己（禁用/管理权限变更；余额调整允许）
    if ($uid === (int)$admin['id'] && $action !== 'balance_adjust') {
        $msg = '不能修改自己的账号';
    } else {
        switch ($action) {
            case 'balance_adjust':
...
// 搜索
$kw = trim($_GET['q'] ?? '');
$sql = 'SELECT id, uid, nickname, email, role, status, balance, created_at FROM users';
$args = [];
if ($kw) {
    ...
    <mdui-list>
      ...
      <mdui-list-item nonclickable>
        ...
        <span slot="description" style="font-size:12px;">
          ...
          · 余额 ¥<?= number_format((int)$u['balance'] / 100, 2) ?>
        </span>

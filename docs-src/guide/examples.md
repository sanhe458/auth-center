...
header('Location: <AUTH_SERVER>/api/oauth/authorize?' . $params);
```
...
// 换令牌
$ch = curl_init('<AUTH_SERVER>/api/oauth/token');
...
// 拿用户信息
$ch = curl_init('<AUTH_SERVER>/api/info');
curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
...
```bash
# 1. 换令牌
curl -X POST <AUTH_SERVER>/api/oauth/token \

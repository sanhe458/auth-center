...
- **撤回授权**：在 [Auth Center 控制台 → 授权管理](https://<AUTH_SERVER>/user/auth.php) 撤回「SDK演示站」的授权，再登录就会重新出现授权页
- **退出登录**：演示站退出只清演示站自己的会话，**不影响你的 Auth Center 账号**

## 体验说明

- 演示站和 Auth Center 是**两个独立域名**（<DEMO_SERVER> 和 <AUTH_SERVER>），会话完全隔离
- 演示站用的应用 `client_id=88020ee0_nm-ouuq`，任何人都可以体验
...
- PHP 版：[sdk/php/examples/](https://<AUTH_SERVER>/sdk/php/examples/login.php)

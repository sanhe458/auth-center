...
(function (global) {
  ...
  function initCaptchaSlider(opts) {
    ...
    function load() {
      loading = true; barText.textContent = '加载中…';
      fetch(API).then(function (r) { return r.json(); }).then(function (d) {
        loading = false;
        if (!d || d.repCode !== '0000') { barText.textContent = '加载失败，点击重试'; return; }
        var rd = d.repData;
        var ts = Date.now();
      ...
      }).catch(function () { loading = false; barText.textContent = '网络错误，点击重试'; });
    }
    // 图片加载失败的兜底
    back.addEventListener('error', function () { if (!state.passed) { setMsg('图片加载失败，换一张', false); setTimeout(load, 800); } });
